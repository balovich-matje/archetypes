"""Contact sheets for the Nemesis-Assassin node icons.
...
the tree screen actually draws it:
  ...
  sheet_16.png     — true 16px on the unowned grey (0x8B8B8B), the hovered
                     grey, an owned plate and the dimmed plate
  sheet_beside.png — the set next to shipped trees, to check it belongs
...
NAMES = ["death_mark", "stalk", "contagion", "headhunter",
         "coup_de_grace", "carrier", "vanishing_act", "deaths_head"]
...
def sheet_true16():
    """One row, drawn at the literal size the tree screen uses, no zoom at
    all — the last honesty check."""
    cell = 20
    im = Image.new("RGBA", (len(NAMES) * cell, cell), GREY)
    for i, n in enumerate(NAMES):
        im.alpha_composite(load(n).resize((16, 16), Image.NEAREST), (i * cell + 2, 2))
    im.save(os.path.join(OUT_DIR, "sheet_true16.png"))


def sheet_beside(trees=("assassin", "shadow", "nemesis_shadow")):
    rows = [("nemesis_assassin", [os.path.join(OUT_DIR, f"{n}.png") for n in NAMES])]
    for t in trees:
    ...
    im = Image.new("RGBA", (130 + cols * (cell + pad), len(rows) * (cell + pad) + pad), PAGE)
